...
def load(app):
  ...
  def review_study_session(id):
    try:
      ...
      # Check if the word is already reviewed in this session
      cursor.execute('''
        SELECT id
      ...
      if review_item:
        return jsonify({"error": "Word already reviewed in this session"}), 400
      ...
      return jsonify({"message": "Word reviewed successfully"}), 201
    except Exception as e:
      return jsonify({"error": str(e)}), 500
